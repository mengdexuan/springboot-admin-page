...
/**
 * 接收邮件并解析
 * @author mengdexuan on 2021/8/10 18:21.
 */
@Slf4j
public class MailUtil {
	...
	public static void resceive() throws Exception {
		...
		store.connect("mengdexuan_test@163.com", "YAIMDENAYOULWVWH"); //163邮箱程序登录属于第三方登录所以这里的密码是163给的授权密码而并非普通的登录密码
		...
		//得到收件箱中的所有邮件并且删除邮件
		markedMsg(Flags.Flag.DELETED,messages);
...
//			log.info("未找到要解析的邮件!");
		...
		for (int i = 0, count = messages.length; i < count; i++) {
			...
			if (isContainerAttachment) {
				saveAttachment(msg, "temp_" + i + "_"); //保存附件
			}
	 ...
	 * @throws IOException
	 */
	public static void markedMsg(Flags.Flag flag,Message... messages) throws MessagingException, IOException {
		if (messages == null || messages.length < 1) {
			//log.info("未找到要解析的邮件!");
			return;
		}
...
//			log.info("Marked {} for message: {}",flag,subject);
		}
	}
